  ...
  normalize: list =>
    list.map(item => {
      if (!item.last_name) {
        ...
        item.full_name = (
          <span>
            <b>{item.last_name}</b>, {item.first_name}
          </span>
        )
      }
      return item
...
changeView.fieldsets = [
  ...
  },
  {
    ...
    description: () => {
      if (crudl.auth.user == crudl.path._id) {
        return (
          <span style={{ color: '#CC293C' }}>
            WARNING: If you remove crudl access for the currently logged-in
            user, you will be logged out and unable to login with this user
            again.
          </span>
        )
      }
  ...
  },
  {
    ...
    expanded: false,
    description:
      'This is an example of a custom field (see admin/fields/SplitDateTimeField.jsx).',
    fields: [
      {
        name: 'date_joined',
        label: 'Date joined',
  ...
  },
  {
    ...
    description:
      "Raw passwords are not stored, so there is no way to see this user's password, but you can set a new password.",
    fields: [
      {
        name: 'password',
        label: 'Password',
