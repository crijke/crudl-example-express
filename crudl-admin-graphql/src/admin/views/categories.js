    ...
    list: function(req) {
      return categories.read(req).then(res => {
        let promises = res.map(item =>
          entries.read(crudl.req().filter('category', item._id))
        )
...
var changeView = {
  ...
  actions: {
    get: function(req) {
      return categories(crudl.path._id).read(req)
    },
    delete: function(req) {
      return categories(crudl.path._id).delete(req)
    },
    save: function(req) {
      return categories(crudl.path._id).update(req)
    }
  }
}
