...
const entryFields =
  '_id, title, status, date, sticky ' +
  'summary, body ' +
  'createdate, updatedate ' +
  'section{_id, name}, category{_id, name}, owner{_id, username}, tags{_id}'
const entries = createResourceConnector('entries', entryFields).use(
  continuousPagination(20)
) // page limit 20

const links = createResourceConnector(
  'entryLinks',
  '_id, title, url, entry{_id}'
)
  ...
  actions: {
    list(req) {
      return entries.read(req).then(res => {
        ...
        return Promise.all(promises).then(entryLinks => {
          res.forEach((item, index) => {
            item.isOwner =
              item.owner && '_id' in item.owner
                ? crudl.auth.user == item.owner._id
                : false
      ...
      onChange: [
        {
          in: 'section',
          setValue: section =>
            section.value !== section.initialValue ? '' : undefined,
          setProps: section => {
            if (!section.value) {
              return {
                readOnly: true,
                helpText:
                  'In order to select a category, you have to select a section first'
              }
            }
...
const changeView = {
  ...
  actions: {
    get(req) {
      return entries(crudl.path._id).read(req)
    },
    delete(req) {
      return entries(crudl.path._id).delete(req)
    },
    save(req) {
      return entries(crudl.path._id).update(req)
    }
  },
  normalize: get => {
...
changeView.fieldsets = [
  {
    fields: [
      ...
      },
      {
        ...
        required: false,
        lazy: () =>
          sectionOptions.read(crudl.req()).then(res => ({
